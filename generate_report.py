...
import matplotlib
matplotlib.use("Agg")  # non-GUI backend (works in Codespaces / servers)
...
def get_git_metrics(interval="weekly"):
    ...
    Returns: students, timeline_activity, student_logs, scope_title
    """
    ...
    # timeline_activity[sort_key] = {"label": str, "authors": {author: count}}
    timeline_activity = {}
    ...
    for line in raw_output.strip().split('\n'):
        ...
        if line.startswith('COMMIT|||'):
            ...
            students[author]["active_days"].add(date_str)
            student_logs[author].append((date_str, sha, msg))
            ...
            try:
                ...
                if interval == "weekly":
                    ...
                    iso = dt.isocalendar()
                    sort_key = f"{iso[0]}-W{iso[1]:02d}"
                    label = sort_key
                else:
                    sort_key = dt.strftime("%Y-%m")
                    label = sort_key
                entry = timeline_activity.setdefault(sort_key, {"label": label, "authors": defaultdict(int)})
...
def create_charts(students, timeline_activity, interval):
    ...
    sorted_keys = sorted(timeline_activity.keys())
    labels = [timeline_activity[k]["label"] for k in sorted_keys]

    # 1. Timeline Line Chart
    if sorted_keys and authors:
        for author in authors:
            counts = [timeline_activity[k]["authors"].get(author, 0) for k in sorted_keys]
            ax1.plot(labels, counts, marker='o', linewidth=2, label=author)
    ...
    if authors:
        ...
        bar_colors = [palette[i % len(palette)] for i in range(len(authors))]
...
def generate_pdf(interval="weekly"):
    ...
    story.append(Paragraph(f"<u><b>{html.escape(report_title)}</b></u>", title_style))
    story.append(Spacer(1, 3))

    # 2. Metadata (Repo, Branch, Scope, Date)
    story.append(Paragraph(
        f"<b>Project Repository:</b> <font color='#2563EB'><b>{html.escape(repo_name)}</b></font>"
        f" &nbsp;|&nbsp; <b>Branch:</b> <font name='Courier'>{html.escape(branch_name)}</font>",
        repo_style))
    story.append(Paragraph(
        f"<b>Evaluation Window:</b> {html.escape(scope_title)} &nbsp;|&nbsp; "
    ...
    if students:
        for name, data in students.items():
            ...
            table_data.append([
                name,  # plain string in a Table cell: no HTML escaping needed
    ...
    # 4. Visual Charts
    story.append(Paragraph("2. Visual Trends & Volume".replace("&", "&amp;"), section_style))
    ...
    if not student_logs:
        ...
        for student_name, logs in student_logs.items():
            ...
            student_section.append(Paragraph(
                f"<b>Student:</b> {html.escape(student_name)} &mdash; <i>{len(logs)} commit(s)</i>",
                sub_section_style))
            ...
            log_table_data.append([
                ...
                Paragraph(f"<font name='Courier'>{html.escape(first_sha)}</font>", meta_cell_style),
            ...
            for date_val, sha_val, msg_val in logs[1:]:
                ...
                log_table_data.append([
                    ...
                    Paragraph(f"<font name='Courier'>{html.escape(sha_val)}</font>", meta_cell_style),
                    Paragraph(safe_msg, msg_style),
                    ""
                ])
            ...
            t_style = [
                ...
                ('BACKGROUND', (3, 1), (3, -1), colors.HexColor("#FEF3C7")),  # Accent for marks area
            ]
            # Vertically merge mentor marks column (only when there is more than one commit row)
            if num_rows > 2:
